...
local function extract_important_sentences(text, max_sentences)
  ...
  for _, sentence in
    ipairs(vim.split(text, '[。！？.!?]\\zs', { trimempty = true }))
  do
    if #sentence > 0 then
      ...
      if char_count >= 3 then
        ...
        local memory_keywords =
          { '重要', '记住', '关键', 'note', 'important', 'remember' }
  ...
  for i = #temp_fragments, 2, -1 do
    ...
    if
      not prev.has_punctuation
      and curr.char_len <= 6
      and (prev.char_len + curr.char_len) <= 50
    then
...
function M.extract_memory(arguments, ctx)
  if not config.config.memory or not config.config.memory.enable then
    return {
      error = 'Memory system is not enabled. Please enable memory in chat.nvim configuration.',
    }
  end
  ...
  if arguments.memories then
    ...
    if type(memories_data) == 'string' then
      ...
      if not ok then
        return {
          error = 'Failed to parse memories JSON: ' .. tostring(parsed),
        }
      end
    ...
    for i, mem in ipairs(memories_data) do
      if type(mem) ~= 'table' or not mem.content then
        return {
          error = string.format(
            'Memory at index %d is invalid (must contain "content" field).',
            i
          ),
        }
      end

      local memory_type = mem.memory_type
        or arguments.memory_type
        or detect_memory_type(mem.content)
      local category = mem.category or detect_category(mem.content)

      -- 构建标记内容
      local marked_content =
        string.format('[%s][%s] %s', memory_type, category, mem.content)
      ...
      -- 存储到对应的记忆系统
      local memory_id = memory.store_memory(
        ctx.session,
        'system',
        marked_content,
        memory_type
  ...
  elseif arguments.text then
    ...
    for _, sentence in ipairs(sentences) do
      local memory_type = arguments.memory_type
        or detect_memory_type(sentence)
      local category = arguments.category or detect_category(sentence)

      -- 构建标记内容
      local marked_content =
        string.format('[%s][%s] %s', memory_type, category, sentence)
  ...
  if #extracted_memories == 0 then
    return {
      content = 'No memorable information extracted. The text may not contain persistent/reusable content.',
    }
  end
...
@extract_memory memories='[{"content":"事实1","category":"fact"},{"content":"偏好1","category":"preference"}]'
      ...
      parameters = {
        ...
        properties = {
          text = {
            type = 'string',
            description = 'Text to analyze (auto-extraction mode)',
          },
          memories = {
            type = 'array',
...
function M.info(action, ctx)
  ...
  elseif arguments.memories then
    local count = type(arguments.memories) == 'table' and #arguments.memories
      or 0
