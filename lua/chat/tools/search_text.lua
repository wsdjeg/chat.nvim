...
function M.search_text(action, ctx)
  ...
  if type(config.config.allowed_path) == 'table' then
    for _, v in ipairs(config.config.allowed_path) do
      if type(v) == 'string' and #v > 0 and starts_with(search_directory, v) then
        is_allowed_path = true
        break
      end
    end
  elseif type(config.config.allowed_path) == 'string' and #config.config.allowed_path > 0 then
    is_allowed_path = starts_with(search_directory, config.config.allowed_path)
  end
  
  if not is_allowed_path then
    return {
      error = string.format('Cannot search in non-allowed path: %s\nAllowed paths: %s', 
                           search_directory,
                           type(config.config.allowed_path) == 'table' and 
                           table.concat(config.config.allowed_path, ', ') or 
                           config.config.allowed_path),
    }
  end
...
function M.info(action, ctx)
  ...
  if ok then
    ...
    local options = {}
    if arguments.ignore_case then table.insert(options, 'ignore_case') end
    if arguments.whole_word then table.insert(options, 'whole_word') end
    if arguments.regex == false then table.insert(options, 'no_regex') end
    if arguments.max_results then table.insert(options, 'max=' .. arguments.max_results) end
    if arguments.context_lines then table.insert(options, 'context=' .. arguments.context_lines) end
