...
function M.plan(arguments, ctx)
  ...
  if action == 'create' then
    ...
    return {
      content = string.format(
        ...
        plan.title, plan.id, #plan.steps, plan.id
      ),
    }
  end
  ...
  if action == 'show' then
    ...
    local output = {
      ...
      string.format('**Created:** %s', os.date('%Y-%m-%d %H:%M', plan.created_at)),
      '',
      '## Steps:',
    }
    ...
    for _, step in ipairs(plan.steps) do
      local status_icon = step.status == 'completed' and '✅'
        ...
        or '⬜'
      local step_line = string.format('%s **%d.** %s', status_icon, step.id, step.content)
  ...
  if action == 'list' then
    ...
    for _, plan in ipairs(plans) do
      ...
      table.insert(output, string.format(
        '- **%s** (`%s`) - %s [%s]',
        plan.title, plan.id, plan.status, progress
      ))
    end
  ...
  if action == 'next' then
    ...
    return {
      content = string.format(
        ...
        step.id, step.content, plan_id, step.id
      ),
    }
  end
  ...
  if action == 'done' then
    ...
    if is_plan_done then
      content = content .. '\n\n🎉 **All steps completed!**\n' ..
        'Use `@plan action="review" plan_id="' .. plan_id .. '"` to add review.'
    end
  ...
  if action == 'review' then
    ...
    local plan, err = plan_module.review_plan(plan_id, summary, lessons, issues)
    if not plan then
      return { error = err }
    end
...
@plan action="review" plan_id="plan-20250110-xxxx" summary="Feature implemented" lessons=["Lesson 1"]
      ...
      parameters = {
        ...
        properties = {
          action = {
            type = 'string',
            enum = { 'create', 'show', 'list', 'add', 'next', 'done', 'review', 'delete' },
          ...
          step_content = { type = 'string', description = 'Step content (for add)' },
          step_id = { type = 'integer', description = 'Step ID (for done)' },
          notes = { type = 'string', description = 'Notes for step completion' },
          status = { type = 'string', description = 'Filter by status (for list)' },
          summary = { type = 'string', description = 'Plan summary (for review)' },
          lessons = {
            type = 'array',
            items = { type = 'string' },
