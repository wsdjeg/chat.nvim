...
function TestIntegration:setUp()
  ...
  self.mock_ims = {
    discord = {
      current_session = function() return 'session-001' end,
    ...
    dingtalk = {
      current_session = function() return 'session-002' end,
      send_message = function(content)
        self.dingtalk_sent = content
      end,
    },
    wecom = {
      current_session = function() return 'session-003' end,
      send_message = function(content)
...
function TestIntegration:testOnResponseWithAllMatchingSessions()
  ...
  local all_same_ims = {
    discord = {
      current_session = function() return 'shared-session' end,
      send_message = function(content) self.discord_sent = content end,
    },
    lark = {
      current_session = function() return 'shared-session' end,
      send_message = function(content) self.lark_sent = content end,
    },
    dingtalk = {
      current_session = function() return 'shared-session' end,
      send_message = function(content) self.dingtalk_sent = content end,
    },
    wecom = {
      current_session = function() return 'shared-session' end,
      send_message = function(content) self.wecom_sent = content end,
    },
    telegram = {
      current_session = function() return 'shared-session' end,
      send_message = function(content) self.telegram_sent = content end,
    },
  }
  ...
  for _, session_data in ipairs(sessions) do
    ...
    for _, im in ipairs({ 'discord', 'lark', 'dingtalk', 'wecom', 'telegram' }) do
      ...
      lu.assertEquals(
        actual, 
        expected, 
        string.format('Session %s: Expected %s to %s', 
          session_data.id, im, expected and 'receive' or 'not receive')
      )
    end
  end
end
